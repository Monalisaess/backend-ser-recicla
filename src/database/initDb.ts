...
export default async function initializeDatabase(): Promise<void> {
  ...
  try {
    ...
    if (!appConfig || !appConfig.is_initialized) {
      logger.info(
        "Banco de dados não inicializado ou flag desativada. Iniciando processo de inicialização...",
      );
      ...
      if (appConfig) {
        ...
        logger.info(
          "Flag de inicialização do banco de dados atualizada para TRUE.",
        );
      } else {
        ...
        logger.info(
          "Registro de configuração de inicialização criado e definido como TRUE.",
        );
      }
      logger.info(
        "Processo de inicialização do banco de dados concluído com sucesso!",
      );
    } else {
